...
Each domain folder holds exactly two files: `queries.ts` and `mutations.ts`.
...
Hotel and tour payment operations live in `src/graphql/ecommerce/mutations.ts` and `src/graphql/ecommerce/queries.ts`.
